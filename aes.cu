...
__device__ void sub_bytes_gpu(uint8_t* bytes, int len_bytes, uint8_t* sbox_gpu) {
    bytes[threadIdx.x] = sbox_gpu[bytes[threadIdx.x]];
    bytes[threadIdx.x + 4] = sbox_gpu[bytes[threadIdx.x + 4]];
    bytes[threadIdx.x + 8] = sbox_gpu[bytes[threadIdx.x + 8]];
    bytes[threadIdx.x + 12] = sbox_gpu[bytes[threadIdx.x + 12]];
}
...
__device__ void shift_rows(uint8_t* bytes) {
    // for (int i = 0; i < 4; i++) {
        for (int j = 0; j < threadIdx.x; j++) {
            uint8_t temp = bytes[0 + threadIdx.x];
            bytes[0 + threadIdx.x] = bytes[4 + threadIdx.x];
            bytes[4 + threadIdx.x] = bytes[8 + threadIdx.x];
            bytes[8 + threadIdx.x] = bytes[12 + threadIdx.x];
            bytes[12 + threadIdx.x] = temp;
        }
    // }
}
...
__device__ void mix_columns(uint8_t* bytes) {
    ...
    // for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            storage[j + (4 * threadIdx.x)] = product(mix_columns_matrix[0][j], bytes[(threadIdx.x * 4)]) ^ 
                product(mix_columns_matrix[1][j], bytes[(threadIdx.x * 4) + 1]) ^ 
                product(mix_columns_matrix[2][j], bytes[(threadIdx.x * 4) + 2]) ^
                product(mix_columns_matrix[3][j], bytes[(threadIdx.x * 4) + 3]);
        }
    // }
    for (int i = 0; i < 4; i++) {
        bytes[(4 * threadIdx.x) + i] = storage[(4 * threadIdx.x) + i];
    }
}

__device__ void add_round_key(uint8_t* bytes, uint8_t* key_section) {
    bytes[threadIdx.x] ^= key_section[threadIdx.x];
    bytes[threadIdx.x + 4] ^= key_section[threadIdx.x + 4];
    bytes[threadIdx.x + 8] ^= key_section[threadIdx.x + 8];
    bytes[threadIdx.x + 12] ^= key_section[threadIdx.x + 12];
}
...
int main(int ac, char** av) {
    ...
    encrypt<<<num_blocks, 4>>>(test, aes, length, sbox);
